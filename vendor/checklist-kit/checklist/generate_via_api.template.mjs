// One-off: generates a QA checklist directly via Google Sheets API by
// executing a `.gs` generator through an Apps-Script-to-Sheets-API adapter.
// Auth: reuses the `google-sheets` MCP OAuth client (Drive + Sheets scopes).
//
// To create a new checklist project:
//   1. Copy this file to <Project>/generate_via_api.mjs
//   2. Edit GS_PATH and GENERATOR_FN to point to your `.gs` file/function
//   3. Edit FOLDER_PATH and FILE_NAME to match the .gs
//   4. Run `node generate_via_api.mjs` from anywhere; MCP_DIR is auto-resolved
//
// Multi-user support: MCP_DIR is found by walking up from CWD looking for
// .mcp.json with mcpServers["google-sheets"].args[0] (falls back to env var
// MCP_SHEETS_DIR). Whoever owns token.json is whose Drive the script writes
// to. A new user must run `node server.mjs --auth` once inside their own
// mcp-sheets clone to bind the script to THEIR Google account.
...
const GENERATOR_FN  = 'create<Project>HomeChecklist';
...
// ── Drive: folder chain (visible subset under drive.file scope) ────────
...
// ── Spreadsheet: find or create + full reset (Vadym, 16/06/2026: also
// strip ALL existing column/row groups so re-runs don't stack toggles) ──
let ssId, sheetId;
...
if (existing.data.files.length) {
  ...
  if (!sh) {
    const r = await api.spreadsheets.batchUpdate({ spreadsheetId: ssId, requestBody: { requests: [
      { addSheet: { properties: { title: 'Checklist', gridProperties: { rowCount: 200, columnCount: 25 } } } }
    ] } });
    ...
    reqs.push({ updateBorders: {
      range: { sheetId, startRowIndex: 0, endRowIndex: 200, startColumnIndex: 0, endColumnIndex: 25 },
  ...
  const created = await api.spreadsheets.create({
    requestBody: {
      ...
      sheets: [{ properties: { title: 'Checklist', gridProperties: { rowCount: 200, columnCount: 25 } } }]
    },
...
const sheetObj = {
  getMaxRows: () => 200, getMaxColumns: () => 25,
  getRange(...args) {
...
const probes = ['Checklist!H7', 'Checklist!I6', 'Checklist!I3', 'Checklist!C5', 'Checklist!E3'];
...
const [h7b, i6b, i3b, c5b, e3b] = await readCells(probes);
console.log('Reverted:', JSON.stringify({ h7: h7b, i6: i6b, i3: i3b, c5: c5b, e3: e3b }));
