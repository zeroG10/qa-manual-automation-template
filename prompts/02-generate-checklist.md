...
3. **Every checklist item MUST start with a stable ID** in the format `[CHK-<FEATURE>-<NNN>]`:
   - `<FEATURE>` is the uppercased feature code derived from the filename. Mapping: `authentication → AUTH`, `managers → MGR`, `field-technicians → FT`, `surveys → SURV`, `templates → TMPL`, `responses → RESP`, `photo-tags → PTAG`. For other names, use the first 4 letters uppercase.
